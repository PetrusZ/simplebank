...
import (
	...
	mockdb "github.com/PetrusZ/simplebank/db/mock"
	...
	mockwk "github.com/PetrusZ/simplebank/worker/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore, taskDistributor *mockwk.MockTaskDistributor) {
				...
				taskDistributor.EXPECT().
					DistributorSendVerifyEmail(gomock.Any(), taskPayload, gomock.Any()).
					Times(1).
					Return(nil)
			},
			checkResponse: func(t *testing.T, resp *pb.CreateUserResponse, err error) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore, taskDistributor *mockwk.MockTaskDistributor) {
				...
				taskDistributor.EXPECT().
					DistributorSendVerifyEmail(gomock.Any(), gomock.Any(), gomock.Any()).
					Times(0)
			},
			checkResponse: func(t *testing.T, resp *pb.CreateUserResponse, err error) {
				require.Error(t, err)
